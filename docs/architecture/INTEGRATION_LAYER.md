...
> 2026-03 增量：skill 已拆分为 `openclaw-skill`（任务）与 `openclaw-files-skill`（文件）；Gateway `/gateway/*` 代理族已扩展文件端点；Dashboard 新增 Cloud Files 与 ZIP 批量下载预览。
